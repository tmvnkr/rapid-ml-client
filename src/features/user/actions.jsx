...
import { asyncActionError, asyncActionStart, asyncActionFinish } from '../async/actions'
...
export const updateProfile = user => {
  return async (dispatch, getState, { getFirebase }) => {
    ...
    try {
      ...
      toastr.error('Error', 'Something went wrong, profile has not been updated.')
    }
  }
}
...
export const cancelGoingToEvent = event => {
  return async (dispatch, getState, { getFirestore }) => {
    ...
    try {
      ...
      toastr.success('Success', `You are no longer interested in ${event.title}`)
    } catch (error) {
  ...
  return async (dispatch, getState) => {
    ...
    switch (activeTab) {
      case 1: // Newest Interested
        query = eventsRef.where('userUid', '==', userUid).orderBy('interestDate', 'desc')
        break
      case 2: // Oldest Collections
        query = eventsRef.where('userUid', '==', userUid).orderBy('eventCreated', 'asc')
        break
      case 3: // Created
        query = eventsRef
      ...
      default:
        // All
        query = eventsRef.where('userUid', '==', userUid).orderBy('eventCreated', 'desc')
    }
    try {
