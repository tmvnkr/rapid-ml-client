...
import { asyncActionStart, asyncActionFinish, asyncActionError } from '../async/actions'
...
export const createEvent = event => {
  return async (dispatch, getState, { getFirestore }) => {
    ...
    try {
      ...
      await firestore.set(`collection_interested/${createdEvent.id}_${user.uid}`, {
...
export const uploadImage = (file, fileName, event) => async (
  ...
  try {
    ...
    await request
      .get(`https://api.imagga.com/v2/tags?image_url=${encodeURIComponent(imageUrl)}`, function(
        error,
        response,
        body
      ) {
